...
function makeAttackedEntry(): Extract<TurnLogEntry, { type: "attacked" }> {
  const attack = getUnitDef("skeleton").attacks[0];
  ...
  return {
    ...
    replay: {
      attacker: { unitDefId: "skeleton", owner: 1, pos: { x: 10, y: 4 }, hp: 34, maxHp: 34 },
...
describe("replayToPlayback", () => {
  it("縮小DTOから再生入力(CombatPlaybackInput)を復元する", () => {
    ...
    expect(input.attacker).toMatchObject({
      id: "sk1", unitDefId: "skeleton", owner: 1, pos: { x: 10, y: 4 }, hp: 34, maxHp: 34,
    });
    ...
    expect(input.attackerAttackId).toBe(getUnitDef("skeleton").attacks[0].id);
